            ...
            <ul class="tabs padding">
                <li class="tab-item" v-for="item in tabs" :key="item.active" @click="checkTab(item)">
                    ...
                    <span v-if="item.active === 5" class="query-icon">
                        <van-icon name="arrow-up" size="0.3rem" v-show="queryPopupShow"></van-icon>
                        <van-icon name="arrow-down" size="0.3rem" v-show="!queryPopupShow"></van-icon>
                    </span>
                </li>
            </ul>
            <!--筛选弹出框-->
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getList () {
                this.loading = true;
                this.$axios.post('/shoping/list', {}).then((res) => {
...
.home{
    ...
    .list-top-box{height: 2.02rem;width: 100%;}
    ...
    .tabs{display: flex;height: 0.88rem;line-height: 0.88rem;border-bottom: 1px solid #eff3f6;border-top: 1px solid #eff3f6;
        .tab-item{width: 20%;width: border-box;font-size: 0.25rem;position: relative}
        .activeTab{color: #ec4125}
        .price-icon{width: 14px;line-height: 0;position: absolute;top: 0.2rem;bottom: 0;
        left: 0.6rem;margin: auto;
            .activeIcon{color: #f82222}
        }
        .query-icon{padding-left: 0.04rem;position: absolute;transform: translateY(0.03rem)}
    }
    .query-dialog{
        .title{line-height: 0.76rem;color: #666666;font-size: 0.28rem;}
        .input-box{padding-bottom: 0.9rem;
            input{width: 2.3rem;height: 0.48rem;line-height: 0.48rem;border-radius: 0.04rem;border: 1px solid #d2d2d2;}
        }
        .button-box{
            .van-button{width: 50%;box-sizing: border-box;height: 0.9rem;}
        }
    }
    .van-list{display: flex;flex-wrap:wrap;justify-content:space-between;}
    .list-item{width: 49%;text-align: left;
        .img-box{min-height: 46vw;width: 100%;}
        .tag-list{padding: 0.16rem 0 0.09rem 0;height: 40px;}
        .van-tag{margin-right: 0.08rem;}
        .tag-1{background-color: #e83632!important;}
        .tag-2{background-color: #c3a25e!important;}
        .tag-3{background-color: #ffba00!important;}
        .integral{padding-top: 0.2rem;padding-bottom: 0.4rem;
            .num{color: #ee5740;}
        }
    }
    .text-overflew{
...
.van-notify{z-index: 4000!important;}
</style>